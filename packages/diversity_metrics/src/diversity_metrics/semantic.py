# Lifted verbatim from whitespace_2/src/whitespace2/semantic_metrics.py (Originality WS2) — behaviour-preserving copy.
"""Semantic-diversity metrics on ``(N, D)`` embedding matrices (or a cluster-
assignment vector).

  - ``effective_dimensionality`` — PCA participation ratio.
  - ``mean_pairwise_cosine_distance`` — mean ``1 - cos`` over vector pairs.
  - ``cluster_entropy`` — Miller-Madow-corrected Shannon entropy of a K-means
