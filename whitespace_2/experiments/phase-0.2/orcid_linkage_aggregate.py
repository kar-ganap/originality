...
- "yes" → 1.0 (correct linkage)
- "likely" → 1.0 (correct linkage; user judged confidently)
- "unclear" → 0.0 (excluded from rate; counted as denominator)
- "no" → 0.0 (incorrect linkage; counted as denominator)
- (empty) → row not yet audited; excluded from BOTH numerator and
...
def _aggregate_rate(
    ...
    Denominator: count of correct + incorrect + unclear rows
    (i.e., audited rows; excludes empty / unknown).
    """
    ...
    for key, sub in groups:
        ...
        n_audited = n_correct + n_incorrect + n_unclear
        ...
        rate = float(n_correct / n_audited) if n_audited > 0 else 0.0
        rows.append({
...
def main() -> None:
    ...
    print("=== Overall ===")
    print(
        f"  audited: {int(overall_row['n_audited'])}/"
        f"{int(overall_row['n_total'])}; "
    ...
    print("=== Per-region (heuristic; user may have re-classified) ===")
    print(
        f"{'region':<13} {'n_total':>8} {'n_audited':>10} "
        f"{'rate':>8} {'status':>8}"
    )
    print("-" * 50)
    for _, row in per_region.sort_values("rate").iterrows():
        ...
        print(
            ...
            f"{int(row['n_audited']):>10} {rate_str:>8} {row['status']:>8}"
        )
    print()
    ...
    md_lines = [
        ...
        f"**N rows:** {len(df)} (audited: "
        f"{int(overall_row['n_audited'])}; "
        f"unaudited: {n_unaudited}; unknown values: {n_unknown})",
        "",
        "## Overall linkage-correctness rate",
        "",
        f"- **{overall_rate:.1%}** ({int(overall_row['n_correct'])} correct / "
        f"{int(overall_row['n_audited'])} audited)",
        f"- §4 threshold: ≥{_THRESHOLD_OVERALL:.0%}",
        f"- **Result: {overall_status}**",
        "",
        "## Per-region rate",
        "",
        "| Region | N total | N audited | Rate | Status |",
        "|---|---:|---:|---:|---|",
    ]
    for _, row in per_region.sort_values("rate", ascending=False).iterrows():
        md_lines.append(
            f"| {row['group']} | {int(row['n_total'])} | "
            f"{int(row['n_audited'])} | {row['rate']:.1%} | {row['status']} |"
        )
    md_lines += [
        "",
    ...
    summary = {
        "snapshot": snapshot,
        "n_total": int(len(df)),
        "n_audited": int(overall_row["n_audited"]),
        "n_unaudited": int(n_unaudited),
        "n_unknown": int(n_unknown),
