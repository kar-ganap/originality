...
def main() -> None:
    ...
    lines.append("")
    lines.append("**Reviewer task:** for each paper below, check:")
    lines.append("")
    lines.append(
        "1. **Field**: is the paper plausibly cs or physics? "
        "(if it's primarily another field that just mentions a cs/physics "
        "term, FLAG)"
    )
    lines.append(
        "2. **Abstract**: is the reconstructed abstract real prose, not "
        "garbled / OCR slop / index artifacts?"
    )
    lines.append(
        "3. **Pre-1990 junk tokens**: for stratum=pre1990, does the "
        "abstract contain post-2000 tokens (e.g., \"2024\", \"GPT\", "
        "\"COVID\", \"ChatGPT\")? FLAG if yes."
    )
    lines.append("")
    lines.append(
        "Mark each paper with `OK` / `FLAG: <reason>` in the line below "
        "the abstract. Pass criterion: **0 FLAGs** out of 100."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
