...
def main() -> None:
    ...
    lines.append(
        ...
        "**Important:** \"belongs in\" means the paper itself (title + "
        "abstract) uses cs/physics methods, ideas, or systems — not "
        "just that a cs/physics concept appears in the concept list. "
        "Every paper in this sample has at least one cs/physics "
        "concept ≥0.30 by construction (that's how it got into the "
        "sample); seeing one in the list is not on its own evidence of "
        "fit. Example: a Croatian urban-planning paper that uses "
        "LiDAR + SLAM + 3D point clouds is **OK** (cs methods doing "
        "the work); an education-research paper with "
        "`computer science (0.34)` tagged but no cs content in the "
        "abstract is **WRONG_FIELD** (the tag is concept-tagger noise)."
    )
    lines.append(
