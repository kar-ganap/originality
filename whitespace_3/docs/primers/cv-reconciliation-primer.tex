...
\medskip\noindent\textbf{Amendment (2026-07-07) --- Core Claim~\ref{cc:open}
disconfirmed; superseded by the content channel.} The per-capita-$V$ extension was run
(WS2 Phase~2.4). $V^{\mathrm{struct}}$ does \emph{not} decline: it \textbf{rises}
($dV^{\mathrm{struct}}/dt>0$; off-canon share $+0.006$ and reference-atypicality
median-$z$ $-0.64$, both $p{=}0.0005$, team- and volume-controlled), and the rise is
\textbf{cross-subfield}---within a subfield combination-novelty is flat, only against the
whole field does novelty rise. This falsifies the \emph{CD-mechanism reading} (that
$\kappa$'s rise \emph{suppresses} per-capita structural novelty), exactly as the
\emph{Status} line anticipated. It does \textbf{not} break the reconciliation---it selects
the \emph{orthogonality} branch. WS3 rung~4d (\texttt{subfield.py}) reproduces the observed
sign from a \textbf{content/$\tau$ channel}: niches recombining a shared canon, niche count
$K=N/m$ growing with scale, which drives $V^{\mathrm{struct}}\!\uparrow$ as a
\emph{fragmentation} effect \emph{orthogonal} to the attachment channel's $H\!\uparrow$
(consistent with $\tau$ being ``to first order ungoverned by $\kappa$,''
\S\ref{sec:kappa}). Corrected picture: the field \textbf{concentrates at the top}
($H\!\uparrow$, attachment/$\kappa$) and \textbf{fragments in the middle}
($V^{\mathrm{struct}}\!\uparrow$, content/$\tau$) \emph{at once}. The model's
$\kappa$-crossover (Core Claim~\ref{cc:wwe}) survives as a \emph{theoretical} capability,
but it is \textbf{not} the empirical driver of the $V^{\mathrm{struct}}$ trend---the
content channel is. See \texttt{whitespace\_2/docs/phases/phase-2.4-retro.md} and
\texttt{whitespace\_3/docs/phases/phase-1-rung4d-retro.md}.

\medskip\noindent\textbf{Amendment (2026-07-24, propagating the 2026-07-22
correction) --- the $\kappa$-crossover is not statistically present at scale; the
Pareto direction is $\partial V>0,\ \partial C=0$, not a both-boost.} A pre-registered
resolution-map study (\texttt{docs/resolution-map-phase3-retro.md}) and the
program-level audit revised Core Claims~\ref{cc:wwe} and~\ref{cc:reconcile} \emph{as
measured phenomena}. (i)~At scale ($N{=}200$--$1500$) the total-$V$ slope in $\log N$
has a bootstrap CI that \textbf{straddles zero} at $\lambda\in\{0,0.1,0.25\}$: there is
no reliably $V$-favouring region, so $\lambda^\ast$ is not a usable quantity and the
$V$-half of ``same lever, opposite signs'' is absent. (ii)~$C$ is a \textbf{deterministic
clock} ($\partial C/\partial\log N = 0$ exactly, zero seed variance), so the Henrich
$C\!\uparrow$ half is absent at scale too. Core Claim~\ref{cc:reconcile}'s \emph{Status}
line (``the paper's central result'') therefore does \textbf{not} stand as an
\emph{empirical} result---the crossover survives only as a \emph{theoretical} capability
of the model (consistent with the 2026-07-07 amendment above, which already removed it as
the empirical driver of the $V^{\mathrm{struct}}$ trend). (iii)~The Pareto clause
``$\exists$ intervention with $\partial C^\ast>0,\ \partial V^\ast>0$'' is corrected to
$\partial V^\ast>0,\ \partial C^\ast=0$: because $C$ is a clock, selective isolation raises
$V$ at \emph{no} $C$-cost (Pareto-\emph{non-worsening}, not a both-boost); the committed
model gives $\partial C/\partial\iota=0$ exactly (\texttt{phase\_diagram.py} Panel~C:
$C$ flat across $\iota$). What survives is the two-channel \emph{sign structure}
($W\!\uparrow \perp V^{\mathrm{struct}}\!\downarrow$ in $N$), not the crossover magnitude
and not a strict both-boost. See the \textsc{correction 2026-07-22} box in
\texttt{whitespace\_3/CLAUDE.md} and \texttt{docs/resolution-map-phase3-retro.md}.
...
\smallskip\noindent\emph{Superseded (2026-07-07):} the final clause is disconfirmed---the
extension (WS2 Phase~2.4) found $V^{\mathrm{struct}}$ \emph{rising}, not declining. Read the
summary as: the $\kappa$-dynamics drives $C\!\uparrow$ and $H\!\uparrow$ on the attachment
channel, while a \emph{second, orthogonal} content/$\tau$ channel (fragmentation, $K=N/m$;
WS3 rung~4d) drives $V^{\mathrm{struct}}\!\uparrow$---concentration \emph{and} fragmentation
at once. See the amendment to Core Claim~\ref{cc:open}.
