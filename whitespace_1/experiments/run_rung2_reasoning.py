"""Fire the catalog-of-reasoning variant on DeepSeek (rung 2, second variant).

    uv run --extra llm python experiments/run_rung2_reasoning.py            # the full 24
    uv run --extra llm python experiments/run_rung2_reasoning.py --limit 3  # smoke one run-id

The R4 loop whose shared catalog holds each agent's reasoning **skeleton** (not its answer): agents
are shown prior *reasoning* and we ask whether their reasoning converges. Extraction is inline (each
round's skeletons feed the next round's catalog), on `deepseek-v4-flash`; generation is thinking-on.
Artifacts carry `skeleton_embeddings` (V_reason, primary) + `proposal_embeddings` (V_output).
Concurrent + resumable to `runs/rung2-reasoning/`. Needs the two keys in the folder-local `.env`.
"""
...
from whitespace1 import rung0_v2
from whitespace1 import rung1_r4 as r4
from whitespace1 import rung2_reasoning as rr
...
class DeepSeekReasoningModeProvider:
    """propose_many builds *reasoning-framed* prompts and captures each trace."""
    ...
    def propose_many(self, *, personas, topic, catalog_samples):  # noqa: ANN001, ANN201
        def one(pair):  # noqa: ANN001, ANN202
            persona, sample = pair
            prompt = rr.build_reasoning_prompt(
                name=persona.name, viewpoint=persona.viewpoint, topic=topic, reasoning_notes=sample
            )
...
def main() -> int:
    ...
    provider = DeepSeekReasoningModeProvider(gen, embedder)
    RUNS_DIR.mkdir(parents=True, exist_ok=True)
    prompt_template = rung0_v2.SKELETON_PROMPTS[SKELETON_PROCEDURE]

    def extract_one(trace: str) -> str:
        if not trace.strip():
            return EMPTY
    ...
    def extract_many(traces: Sequence[str]) -> list[str]:
        with ThreadPoolExecutor(max_workers=PERSONA_WORKERS) as ex:
            return list(ex.map(extract_one, traces))

    specs = _specs()
    todo_all = [s for s in specs if not _artifact_path(s[0], s[2]).exists()]
    todo = todo_all[: args.limit] if args.limit is not None else todo_all
    print(f"rung 2 reasoning: {len(specs) - len(todo_all)} banked, {len(todo)} to run "
    ...
    def run_one(spec: tuple[str, str, r4.R4Condition, int, int]) -> str:
        ...
        run = rr.run_r4_reasoning(provider=provider, config=config, extract_many=extract_many)
        art = rr.r4_reasoning_to_dict(run)
